...
* @desc    LFOSC clock is output via GPIO3. LFOSC时钟通过GPIO3输出。
...
* @desc    AFC enable or disanble. AFC启用或禁用
* @param   bEnable(TRUE): Enable it
*          bEnable(FALSE): Disable it(default)
* *********************************************************/
...
* @desc    This is optional, only needed when using Rx fast frequency hopping 这是可选的，仅在使用Rx快速跳频时才需要.
...
void CMT2300A_Init(void)
{
    ...
    return_dat=CMT2300A_GoStby();
    if(return_dat!=TRUE)
    {
        g_tSystemMsg.CMT_err = SET;
    }
    else {
        g_tSystemMsg.CMT_err = RESET;
    }
    tmp  = CMT2300A_ReadReg(CMT2300A_CUS_MODE_STA);
    tmp |= CMT2300A_MASK_CFG_RETAIN;         /* Enable CFG_RETAIN 0x10*/
    tmp &= ~CMT2300A_MASK_RSTN_IN_EN;        /* Disable RSTN_IN 0x20*/
    CMT2300A_WriteReg(CMT2300A_CUS_MODE_STA, tmp);//保护0x00-0x5f的配置不被软件复位

    tmp  = CMT2300A_ReadReg(CMT2300A_CUS_EN_CTL);
    tmp |= CMT2300A_MASK_LOCKING_EN;         /* Enable LOCKING_EN */
    CMT2300A_WriteReg(CMT2300A_CUS_EN_CTL, tmp);//使能LBD。在检测到设置低电平时，不让ic进入发射护照接收
...
/*! ********************************************************
* @name    CMT2300A_Write_Preamble
* @desc    写preamble 函数
* *********************************************************/
void CMT2300A_Write_Preamble(u8 base_adrr,u8 bank[],u8 len)
{
    u8 i;
     for(i=0; i<len; i++)
    CMT2300A_WriteReg(i+base_adrr, bank[i]);
}
/*! ********************************************************
* @name    CMT2300A_Write_Preamble
* @desc    写preamble 函数
* *********************************************************/
void CMT2300A_Write_nodeval(u8 base_adrr,u8 bank[],u8 len)
{
     u8 i;
     for(i=0; i<len; i++)
    CMT2300A_WriteReg(i+base_adrr, bank[i]);   
