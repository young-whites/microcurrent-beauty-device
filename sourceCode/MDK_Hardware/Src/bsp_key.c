...
void KEY_GPIO_Config(void)
{
	...
	GPIO_CONFIG_IO_MODE(macKEY_Power_GPIO_PORT, macKEY_Power_GPIO_PIN, GPIO_MODE_INPUT_PULL_UP);
	GPIO_DisableInt(macKEY_Power_GPIO_PORT, macKEY_Power_Pin_MSK);
...
/* 以下是按键驱动（视情况使用）**************************************************************************************************************************************************************/
//----------------------------------------------------------------------------
static  INT8U	bPowerShort = 0;					// 短按处理
//----------------------------------------------------------------------------
...
uint8_t KEY_GetState(uint8_t keyName)
{
	...
	switch (keyName)
	{
		case KEY_Val_Power:	stat = (GPIO_GetPin(macKEY_Power_GPIO_PORT, macKEY_Power_Pin_MSK) ? 0 : 1);		break;	// 按键管脚 低电平表示导通 
	}
	return stat;
}
...
* 功能:		电源按键 按下处理
*****************************************************************************/
static void _KEY_Power_Press(void)
{
	bPowerShort = 1;
	BEEP_ShortPress();
...
* 功能:		电源 按键 释放处理
*****************************************************************************/
static void _KEY_Power_Release(void)
{
	Record.PowerKeyCnt++;
	...
	for (key = KEY_Read(); key; key = KEY_Read())
	{
		...
		switch (key)
		{
			...
			case KEY_Val_Power:
			{
				switch (event)
				{
					case KEY_Evt_Press:		_KEY_Power_Press();		break;	// 按下
					case KEY_Evt_Release:	_KEY_Power_Release();	break;	// 松开
				}
			}break;
